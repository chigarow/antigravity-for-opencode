...
Guidelines:
- Choose one of the versioned tiers: `flash-3.6-med` (default Medium), `flash-3.5` (fast/cheap), `flash-3.5-lo` (lower cost), `pro-3.1` (stronger reasoning), `flash-3.6` (newer flash workhorse), or `flash-3.6-lo` (cheapest 3.6 option), based on task complexity.
